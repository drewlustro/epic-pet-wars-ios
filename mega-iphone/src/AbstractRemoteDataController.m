...
@implementation AbstractRemoteDataController
...
- (void)loadInitialData:(BOOL)forceReload showLoadingOverlay:(BOOL)showLoadingOverlay {
    ...
    if ([self isInitialLoadRequired] || forceReload) {
        if (showLoadingOverlay) {
            [self showLoadingOverlay];
        }
        [self performInitialLoadRemoteDataCall];
    }
}
...
- (BOOL)hasInitialLoadBeenAttempted {
    return hasInitialLoadAttemptOccured;
}
...
- (void)performInitialLoadRemoteDataCall {
    hasInitialLoadAttemptOccured = YES;
}
...
- (void)showLoadingOverlay {
    [containerController showLoadingScreen];
}
