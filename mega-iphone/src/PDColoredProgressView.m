...
- (void)animateProgressTo:(float)progress {
    ...
    animationTimer = [[NSTimer timerWithTimeInterval:.01 target:self selector:@selector(animateProgressToHelper:) userInfo:[NSNumber numberWithFloat:progress] repeats:YES] retain];
    [[NSRunLoop currentRunLoop] addTimer:animationTimer forMode:NSDefaultRunLoopMode];
}

- (void)animateProgressToHelper:(NSTimer *)timer {
#define STEP .01
    float progress = [timer.userInfo floatValue];
	if (fabs(self.progress - progress) < STEP) { // progress is basically equal
		self.progress = progress;
		[animationTimer invalidate];
	}
	
	if (self.progress < progress) { // need to increase the progress
		self.progress += STEP;
	} else { // need to decrease the progress
		self.progress -= STEP;
	}
}

- (void)animateProgressToHelper3:(NSNumber *)progress {
	[self animateProgressTo:[progress floatValue]];
}
